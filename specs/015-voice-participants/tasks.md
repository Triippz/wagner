...
>
> **Deferred (workstation/device-gated):** cpal capture/playback (real mic/speaker), wake.rs (needs a trained model T042), the participants' **device wiring** (capture→AEC→STT feeding `voice.utterance_transcribed`; Tts→cpal playback) and **shell registration** (T014b — `AgentRegistry::spawn()` calls in `edge/shell/src/lib.rs` for `VoiceIntake` + `VoiceProjection`, blocked on a concrete `Tts` impl), shell shortcuts + bundling + notarization (T041), wake-word training (T042), UI (T040), and the manual gates (T046). These need real audio devices, mic permission, or a trained model.
...
- [ ] T013 [P] Test: participant scaffolding registers on a fake `AgentRegistry`; start/stop/abort route through it (FR-008), in `edge/host/tests/unit/voice_participant_lifecycle.rs`. **Partially done — the inline `agent_tests` in `voice_intake.rs` test dispatch through a bus context, but the 'registers' criterion is not met: neither `VoiceIntake` nor `VoiceProjection` is passed to `AgentRegistry::spawn()` in any test. Remaining: add a test that calls `registry.spawn(Box::new(VoiceIntake::new(…)))`, publishes a `VoiceEvent::UtteranceTranscribed` through the registry's bus, and asserts a command arrives on the intake receiver.**
- [x] T014a Add `voice_intake` + `voice_projection` `Agent` impls in `edge/host/src/participants/`. **DONE — full Agent impls: `VoiceIntake` + `VoiceProjection` with subscribe/handle implemented and tested (2026-06-22).**
- [~] T014b Register `VoiceIntake` + `VoiceProjection` in `edge/shell` via `AgentRegistry::spawn()` (depends T013, T014a). **`VoiceIntake` DONE (2026-06-22)** — `edge/shell/src/lib.rs` setup builds the registry, then `registry.spawn(Box::new(VoiceIntake::new(ctx, AllowAll, voice)))` (wrapped in `block_on` since spawn uses `tokio::spawn`; `VoiceManager` is now `Arc`-managed and shared). **`VoiceProjection` still open** — needs a concrete `Tts` wired for the shell + cpal playback (device-gated).

> **Live push-to-talk (M1/M2a/M2b, 2026-06-22, on origin/main):** the whole speak→run chain is wired end-to-end (compiles, host 439 green, shell clippy clean; live behaviour pending `make edge`). **M1** `voice/capture.rs` (cpal mic → 16 kHz mono WAV; pure transforms tested, `MicCapture` owns the `!Send` stream on its own thread). **M2a** `voice_ptt_start/stop` commands + hold-to-talk button → capture → `HttpStt` (:8771) → transcript returned to the UI. **M2b (the deferred 011-P4 / command→run bridge):** engine-side `orchestrator::run_router` (`RunLaunch` port + `RunCommandRouter`, 4 headless tests) + shell `ShellRunLaunch` adapter + extracted `launch_run_core` (shared by `start_run` and the adapter); `bus_gateway` now routes dispatched `RunCommand`s to real runs (replaces the log-only drain); `voice_ptt_stop` publishes `UtteranceTranscribed` → `VoiceIntake` → dispatch → router → run. A voice-launched run defaults to app-cwd + default roster (`ponytail:` heuristic). **Architecture recorded in ADR-0004** (engine-owns-run-launch via a port; shell is the adapter). The full engine-side relocation of pool/gate/suite (for a headless daemon) remains future work.
...
- [ ] T016 Implement the shared free-form intake routing in `edge/host/src/participants/voice_intake.rs`: capture(post-AEC)→`Stt`→transcript→run-targeting→validated command via dispatch (depends T014, T015). **Partially done** — the transcript→run-targeting→dispatch path (`route_transcript`/`IntakeAction`/`to_command`) is implemented and tested in `VoiceIntake::act_on_transcript` (this branch, 2026-06-22). Remaining open portion: the capture(post-AEC)→`Stt`→`UtteranceTranscribed` prefix (device-gated; needs real mic + `cpal` capture wired to the AEC+STT pipeline).

### Cross-cutting guarantees (privacy / schema / errors / toggle)
> **2026-06-22 (headless guards, `feat/voice-cross-cutting-guards`):** T019/T020 + the
> headless portions of T017/T018 are implemented + tested green (9 new tests; full host
> suite 429 green, clippy `-D warnings` clean). Inline `#[cfg(test)]` modules per the
> voice-module convention. The capture/playback halves of T020 and the hub-sync half of
> T018 stay device-/path-gated as noted below.
- [x] T017 [P] **FR-013** schema-validation at every voice emit site — **satisfied by construction + existing coverage** (no new code). The only headless voice emit site is `VoiceIntake`, which emits exclusively **typed** `Command`s through `Bus::dispatch` (never a raw-JSON payload). Typed commands are well-formed by the type system; `bus_schema_validate.rs` already (a) validates `Command::Run(RunCommand::Start)` against the command schema and (b) carries a type↔schema **drift guard** that fails if the Rust enum and `command.json` diverge. The device-gated capture path's `voice.utterance_transcribed` emit is the remaining (gated) emit site.
- [x] T018 [P] **FR-012** privacy — **headless data-model guard landed**: `state/store.rs::run_state_contract_admits_no_raw_voice_field` asserts the persisted+syncable run contract (`run-state.schema.json`, `additionalProperties:false`) admits no `audio`/`transcript`/`utterance`/`pcm` field. Raw voice audio/transcript live only on the local bus (`VoiceEvent`), never persisted (store persists `Run`, not bus events); a spoken *goal* is legitimate run data. **Deferred (D-TEST-4):** the full stub-hub run-sync guard lands when the host gains a run-sync-to-hub path (none exists today).
- [x] T019 [P] **FR-014** typed error — mic-denied, STT-down, TTS-down surface a typed `VoiceError` via `VoiceStatus.last_error` (no panic/unwrap). Added `VoiceError::MicDenied`; `VoiceManager::report_error` + `VoiceStatus.last_error` (additive; TS bridge lockstep); `VoiceProjection` reports TTS-down failures. Tests inline in `manager.rs` (3 typed errors + clear-on-enable) and `voice_projection.rs` (TTS-down → surfaced, no panic).
- [x] T020 [P] **FR-015** toggle gate + **EC-006** — voice off ⇒ `VoiceProjection` stays silent on otherwise-allowlisted events and `VoiceIntake` drops an in-flight transcript without dispatch; toggling off mid-stream halts both with no submission (focus bookkeeping continues). Both participants check `VoiceManager::enabled()`. Tests inline in both participants (off-gate + mid-stream toggle-off). The capture/playback-level "no `AudioChunk` captured" half is device-gated (capture.rs/playback.rs not yet built).
- [x] T021 Implement the cross-cutting guards — done for the headless surface (typed-error surfacing + toggle gate across `voice_intake`/`voice_projection`/`manager`; privacy contract guard in `store`). The `capture`/`playback` portions land with the device-gated cpal wiring.
...
- [x] T034 [US3] Implement `edge/host/src/voice/cancel.rs` — best-effort matcher (short utterance, no trailing goal). **DONE** (`classify_spoken`/`SpokenIntent`, clippy clean). Wired into `VoiceIntake::handle` via `route_transcript` on this branch (2026-06-22); the capture→AEC→STT prefix feeding `UtteranceTranscribed` remains device-gated.
...
- [x] T035 [P] [US4] Test (**SC-004 automated**): allowlist — transmission + each milestone → `FakeTts` invoked; a scripted non-allowlisted event stream → **zero** `FakeTts` invocations. **DONE** — `agent_tests::speaks_each_allowlisted_event` + `agent_tests::silent_on_non_allowlisted_events` in `voice_projection.rs` (inline `agent_tests` module; invocation-count via `CountingTts`; not in `tests/unit/voice_projection_allowlist.rs` as originally specified).
...
- [x] T037 [US4] Implement `voice_projection`: subscribe events → allowlist filter → `Tts` → `playback` (depends T014, T035). **DONE** (subscribe→filter→Tts path implemented in `VoiceProjection::handle`, 2026-06-22) — except cpal playback (device-gated, T038).
