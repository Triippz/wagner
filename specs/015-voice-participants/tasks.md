...
- [ ] T014b Register `VoiceIntake` + `VoiceProjection` in `edge/shell` via `AgentRegistry::spawn()` (depends T013, T014a). **Open — `edge/shell/src/lib.rs` builds the registry (line 89) but never calls `registry.spawn()` for either participant. Blocked until a concrete `Tts` impl exists for the shell (device-gated). Deferred-items note has been updated to name this explicitly.**
